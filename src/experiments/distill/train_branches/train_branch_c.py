...
from core.prompts.mmlu_branches_sft import branch_c_sys_prompt, branch_c_user_prompt

logging.basicConfig(level=logging.INFO, format="%(levelname)s: %(message)s")

BASE_MODEL = "Qwen/Qwen2.5-3B-Instruct"
DATA_FILE = "data/out/sft_data/branch_c_train.parquet"
...
def get_sys_prompt(row):
    return branch_c_sys_prompt(row["subject"] if row["subject"] else None)

def get_user_prompt(row):
    options = ast.literal_eval(row["options"])
    return branch_c_user_prompt(row["question"], options, row["prev_answer"], row["gold_answer"])

def prepare_branch_c_dataset(tokenizer, df):
    """Prepare Branch C dataset with error analysis"""
    df_copy = df.copy()
...
train_ds = prepare_branch_c_dataset(tokenizer, df)
